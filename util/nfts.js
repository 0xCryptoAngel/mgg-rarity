...
const set_missing_traits = (nft, missing_traits, all_traits) => {
  ...
  const [firstTrait] = Object.keys(all_traits);
  const totaltraits = all_traits[firstTrait].sum;
  nft["missing_traits"] = [];
...
const set_nft_rarity = (nft, all_traits) => {
  const [firstTrait] = Object.keys(all_traits);
  let sumoftraits = all_traits[firstTrait].sum; //All types humans, aliens
  if (nft) {
    let { attributes } = nft;
    attributes = attributes.filter(
      (attribute) => attribute["trait_type"] && attribute["value"]
...
const calculate_attribute_rarity = (nft) => {
  ...
  const [firstTrait] = Object.keys(all_traits);
  let sumoftraits = all_traits[firstTrait].sum;
  nft["trait_count"] = {
...
export const getNFT = (id) => {
  ...
  let nft = nfts.find((e) => e.id === parseInt(id));
  if (nft) {
